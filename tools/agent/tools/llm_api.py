...
def create_llm_client(provider="openai"):
    if provider == "openai":
        ...
        return {"api_key": api_key, "base_url": "https://api.venice.ai/v1"}
    ...
    try:
        ...
        if model is None:
            if provider == "openai":
                ...
                model = "llama-3.3-70b"  # Default model with function calling and general purpose
            elif provider == "ollama":
                model = "mistral"
        ...
        if provider in ["openai", "local", "deepseek", "azure"]:
            ...
            if image_path:
                ...
                payload["messages"][0]["content"] = [
                    {"type": "text", "text": prompt},
                    {"type": "image", "image_url": f"data:{mime_type};base64,{encoded_image}"}
                ]
...
def main():
    ...
    parser.add_argument('--provider', choices=['openai','anthropic','gemini','local','deepseek','azure','venice','ollama'], default='openai', help='The API provider to use')
